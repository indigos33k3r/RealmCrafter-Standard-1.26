...
Function LogIn()
	...
	TName   = GY_CreateTextField(0, 0.41, 0.29, 0.36, 4, 20) ; account - charname verification
	TPass   = GY_CreateTextField(0, 0.41, 0.35, 0.36, 5, 20, "", True)	; password verification
	TEmail  = GY_CreateTextField(0, 0.41, 0.41, 0.36, 6, 30)
	...
	Select PeerToHost
		Case RCE_HostNotFound : RuntimeError(LanguageString$(LS_InvalidHost) + " (" + ServerHost$ + ")")
		Case RCE_TimedOut : RuntimeError(LanguageString$(LS_NoResponse))
		Case RCE_ServerFull : RuntimeError(LanguageString$(LS_TooManyPlayers))
	End Select
	...
	Repeat
		...
		If GY_ButtonHit(BLogin) = True
			...
			Else
				GY_MessageBox(LanguageString$(LS_Error), LanguageString$(LS_AccountAlreadyConnected)) : Goto Invld
			EndIf
		EndIf
...
Function CreateChar()
	...
	TName = GY_CreateTextField(0, 0.36, 0.86, 0.35, 4, 25)	; Verification Textfield
...
Function GameOptionsMenu()
	...
	Repeat
		...
		If GY_WindowActive(WGraphics) And GraphicsVisible = True
			...
			If GY_CheckBoxHit(BAntiAlias) Or GY_CheckBoxHit(BEnableGrass) Or GY_CheckBoxHit(BFullScreen) Or GY_CheckBoxHit(BVSyncOption) Or ComboChanged
				CurrentRes$ = GY_ListBoxItem(LResolution)
				CurrentDepth$ = GY_ComboBoxItem(CDepth)
				CurrentAnisotropy$ = GY_ComboBoxItem(CAnisotropy)
				; Update file
