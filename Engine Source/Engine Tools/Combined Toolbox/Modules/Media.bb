...
BF_Start(5378) ; TEMPORARY! Remove on conversion to RC 2.0
...
Function GetMesh(ID, Duplicate = False)
	...
	If LoadedMeshes(ID) = 0
		...
		; Load the mesh
		If IsAnim = True
			LoadedMeshes(ID) = TempLoadAnimMesh("Data\Meshes\" + Name$)
		...
		Else
			LoadedMeshes(ID) = TempLoadMesh("Data\Meshes\" + Name$)
			If LoadedMeshes(ID) = 0 Then Return(0)
			HideEntity(LoadedMeshes(ID))
			ScaleEntity(LoadedMeshes(ID), LoadedMeshScales#(ID), LoadedMeshScales#(ID), LoadedMeshScales#(ID))
		EndIf
	...
	Else
		...
		If Lower(Right(Name$, 4)) = ".b3d" Then
			If IsAnim = True
		...
		Else
			If IsAnim = True
				EN = TempLoadAnimMesh("Data\Meshes\" + Name$)
				If EN = 0 Then Return(0)
				ScaleEntity(EN, LoadedMeshScales#(ID), LoadedMeshScales#(ID), LoadedMeshScales#(ID))
			Else
				EN = TempLoadMesh("Data\Meshes\" + Name$)
				If EN = 0 Then Return(0)
				ScaleEntity(EN, LoadedMeshScales#(ID), LoadedMeshScales#(ID), LoadedMeshScales#(ID))
			EndIf
		End If
...
; Temporary mesh loading functions which decrypt .eb3d files
Function TempLoadAnimMesh(Name$, Parent = 0)
	...
	EN = LoadAnimMesh(Name$, Parent)
	If Size > 0 Then DeleteFile(Name$)
	Return EN

End Function
Function TempLoadMesh(Name$, Parent = 0)
