...
Function InventoryDrop(A.ActorInstance, SlotFrom, Amount, TellServer = True)
	...
	; Tell server
	If TellServer = True
		RCE_Send(Connection, RCE_Host, P_InventoryUpdate, "D" + RCE_StrFromInt$(SlotFrom, 1) + RCE_StrFromInt$(Amount, 2), True)
	EndIf
...
Function InventoryAdd(A.ActorInstance, SlotFrom, SlotTo, Amount, TellServer = True)
	...
	If TellServer = True
		...
		RCE_Send(Connection, RCE_Host, P_InventoryUpdate, "A" + Pa$, True)
	EndIf
...
Function InventorySwap(A.ActorInstance, SlotA, SlotB, Amount = 0, TellServer = True)
	...
	If TellServer = True
		...
		RCE_Send(Connection, RCE_Host, P_InventoryUpdate, "S" + Pa$, True)
	EndIf
